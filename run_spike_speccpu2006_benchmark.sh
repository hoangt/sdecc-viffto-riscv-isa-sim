...
if [[ "$MODE" == "faultinj" ]]; then
    CMD="$SPIKE_DIR/spike --faultinj=500:data:$MWG_GIT_PATH/eccgrp-ecc-ctrl/data_recovery_spike_wrapper.sh --memwordsize=8 --ic=1:1:64 --dc=1:1:64 --isa=RV64G -m1024 -p1 $MWG_GIT_PATH/eccgrp-riscv-pk/build/riscv64-unknown-elf/bin/pk ${BENCHMARK_NAME}_base.${SPEC_CONFIG_SUFFIX} $BENCHMARK_ARGS"
fi
